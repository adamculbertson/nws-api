...
DEFAULT_UNIT = "0"  # 0 for imperial, 1 for metric (mostly)
...
DEFAULTS = {
    ...
    "forecast": {
        "unit": "0",  # 0 for imperial, 1 for metric
        "lang": "english"
    },
...
"""
# Example forecast location:
"forecast": {
    "lat": None,
    "lon": None,
    "unit": "0",  # 0 for imperial, 1 for metric (mostly)
    "lang": "english",
    "url": None  # If specified, overrides all other options
},
"hwo": {
    "office": None,  # Force the NWS Office to use for the Hazardous Weather Outlook
    "url": None  # Force the URL used to retrieve the HWO, will override the office setting
}
"""
...
def load(file_path=default_config_file, data=None):
    """
    ...
    if "forecast" in config:
        if "unit" not in config['forecast']:
            config['forecast']['unit'] = DEFAULT_UNIT

        if "lang" not in config['forecast']:
            config['forecast']['lang'] = DEFAULT_LANG

    else:
        raise ConfigError(f"Missing forecast information from {file_path}")
